...
def plot_cumulative_treatment_effect(
    treated_gap,
    placebo_gap,
    treatment_date,
    title="Placebo Test – Cumulative Treatment Effect"
):
    """
    treated_gap  : pd.Series (gap do tratado)
    placebo_gap  : dict[str, pd.Series] (gaps dos placebos)
    treatment_date : data do tratamento
    """

    plt.figure(figsize=(10, 4))

    # Placebos
    for _, series in placebo_gap.items():
        cte = series.copy()
        cte.loc[cte.index < treatment_date] = 0.0
        cte.loc[cte.index >= treatment_date] = (
            series.loc[series.index >= treatment_date].cumsum()
        )
    ...
    # Tratado (destacado)
    treated_cte = treated_gap.copy()
    treated_cte.loc[treated_cte.index < treatment_date] = 0.0
    treated_cte.loc[treated_cte.index >= treatment_date] = (
        treated_gap.loc[treated_gap.index >= treatment_date].cumsum()
    )
